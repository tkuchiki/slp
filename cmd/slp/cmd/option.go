...
func createOptions(rootCmd *cobra.Command, sortOptions *stats.SortOptions) (*options.Options, error) {
	...
	assembleWhereIn, err := rootCmd.PersistentFlags().GetBool("assemble-where-in")
	if err != nil {
		return nil, err
	}

	assembleValues, err := rootCmd.PersistentFlags().GetBool("assemble-values")
	if err != nil {
		return nil, err
	}
	...
	return options.SetOptions(opts,
		...
		options.AssembleWhereIn(assembleWhereIn),
		options.AssembleValues(assembleValues),
